...
driver_sets = {
    "winxp": {
        "netkvm": Driver(
            "XP/x86", "XP/amd64", "(netkvm.*|readme.doc)"
        ),
        "serial": Driver(
            "Wxp/x86", "Wnet/amd64", "(vioser.*|wdfcoinstaller.*.dll)"
        ),
        "balloon": Driver(
            "Wxp/x86", "Wnet/amd64", "(balloon.*|bln.*|wdfcoinstaller.*.dll)"
        ),
    ...
    "win2k3": {
        ...
        "serial": Driver(
            "Wnet/x86", "Wnet/amd64", "(vioser.*|wdfcoinstaller.*.dll)"
        ),
        "balloon": Driver(
            "Wnet/x86", "Wnet/amd64", "(balloon.*|bln.*|wdfcoinstaller.*.dll)"
        ),
    ...
    "vista": {
        ...
        "serial": Driver(
            "Wlh/x86", "Wlh/amd64", "(vioser.*|wdfcoinstaller.*.dll)"
            ),
        "balloon": Driver(
            "Wlh/x86", "Wlh/amd64", "(balloon.*|bln.*|wdfcoinstaller.*.dll)"
        ),
    ...
    "win2k8": {
        "netkvm": Driver(
            "Vista/x86", "Vista/amd64", "(netkvm.*|readme.doc)"
        ),
        "serial": Driver(
            "Wlh/x86", "Wlh/amd64", "(vioser.*|wdfcoinstaller.*.dll)"
        ),
    ...
    "win7": {
        ...
        "serial": Driver(
            "Win7/x86", "Win7/amd64", "(vioser.*|wdfcoinstaller.*.dll)"
        ),
        "balloon": Driver(
            "Win7/x86", "Win7/amd64", "(balloon.*|bln.*|wdfcoinstaller.*.dll)"
        ),
    ...
    "win8": {
        ...
        "serial": Driver(
            "Win8/x86", "Win8/amd64", "(vioser.*|wdfcoinstaller.*.dll)"
        ),
        "balloon": Driver(
            "Win8/x86", "Win8/amd64", "(balloon.*|bln.*|wdfcoinstaller.*.dll)"
        ),
        ...
        for file in os.listdir(src_path):
            if (file_regex.match(file)):
                ...
                try:
                    ...
                    if (
                        (
                            file != "wdfcoinstaller01009.dll" and
                            file != "wdfcoinstaller01011.dll"
                        ) or
...
def copy_drivers(base_src, base_dest, driver_filter=None, copy_debug=False):
    for win, driver_set in driver_sets.iteritems():
        ...
        for driver_name, driver in driver_set.iteritems():
            if driver_filter is not None and driver_name not in driver_filter:
...
def download_iso(dest):
    base_url = (
        ...
        "/groups/virt/virtio-win/deprecated-isos/latest/"
    )
    try:
